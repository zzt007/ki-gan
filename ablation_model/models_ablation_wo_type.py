...
class AttenPoolNet(PoolHiddenNet):
    ...
    def forward(self, h_states, seq_start_end, end_pos, vx, vy, agent_type):
        ...
        for _, (start, end) in enumerate(seq_start_end):
            ...
            # .max(1) 返回每行的最大值及其对应的索引，.max(1)[0]表示获取最大值, .max(1)[1]表示获取最大值对应的索引
            curr_pool_h = curr_pool_h.view(num_ped, num_ped, -1).max(1)[0]
...
class TrajectoryGenerator(nn.Module):
    def __init__(
        ...
        # 做消融实验，把motion除去，即和原来一样，只有encoder_h_dim*2 + bottleneck_dim + traffic_h_dim + spectral_h_dim, 不过encoder_h_dim 在数值上等于 spectral_h_dim
        if pooling_type:
            input_dim = encoder_h_dim*2 + bottleneck_dim + traffic_h_dim + encoder_h_dim  # 不使用motion 编码时
    ...
    def forward(self, obs_traj, obs_traj_rel, seq_start_end,  vx, vy, ax, ay, agent_type, size, traffic_state, user_noise=None,):
        ...
        # 250113 做attr的消融，把vehicle_encoding去掉
        # combined_encoding = torch.cat([final_encoder_h, vehicle_encoding, state_encoding], dim=2)
        combined_encoding = torch.cat([final_encoder_h, state_encoding], dim=2)
        ...
        if self.pooling_type:
            ...
            # 250113，做attr(即vehicle_encoding)的消融实验
            mlp_decoder_context_input = torch.cat(
                [combined_encoding.view(-1, self.encoder_h_dim*2), 
                 pool_h, 
                 spectral_encoding.view(-1, self.embedding_dim), 
                 traffic_encoding.view(-1, self.traffic_h_dim)
                 ], dim=1)           
