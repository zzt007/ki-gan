...
def plot_trajectories_on_map(real_traj, pred_traj, seq_start_end, plot_number):
    ...
    bgcolor = 'white'
    edge_color = 'black'
    # edge_linewidth=1 可调节地图线粗细
    fig, ax = ox.plot_graph(G, bgcolor=bgcolor, edge_color=edge_color, edge_linewidth=0.5,show=False, close=False)
    ...
    for (start, end) in seq_start_end:
        ...
        for i in range(start, end):
            ...
            plt.plot(real_traj[:num_points, i, 1], real_traj[:num_points, i, 0], color=color_real, linewidth=2,linestyle='--')
            ...
            #            color=color_real,scale=0.5
            #           )
            
            # 250115 不绘制箭头，末点采用实心圆点表示
            plt.scatter(real_traj[-1,i,1], real_traj[-1,i,0],color = color_real, marker='*', s=30)
                        
            num_points_pred = pred_traj[13:31].shape[0] # 此处修改是12-12 还是 12-18，当为输出18时，则pred_traj[13:31]；当为12时，则pred_traj[13:25]
            plt.plot(pred_traj[13:31, i, 1], pred_traj[13:31, i, 0], color=color_pred, linewidth=2,linestyle='--')
            ...
            #            color=color_pred,scale=0.5
            #            )
            # 250115 不绘制箭头，末点采用实心圆点表示
            plt.scatter(pred_traj[-1,i,1], pred_traj[-1,i,0],color = color_pred, marker='*', s=30)
    ...
    if not os.path.exists('Result_6s-6s-edge_linewidth0_5'):
        os.makedirs('Result_6s-6s-edge_linewidth0_5')

    plt.savefig(f'Result_6s-6s-edge_linewidth0_5/trajectory_{plot_number}.png',dpi=600)
    # plt.savefig(f'Result_6s-9s-test/trajectory_{plot_number}.svg',format='svg')
    plt.close(fig)
